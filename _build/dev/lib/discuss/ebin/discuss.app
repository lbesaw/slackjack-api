{application,discuss,
             [{description,"discuss"},
              {modules,['Elixir.Discuss','Elixir.Discuss.ApiController',
                        'Elixir.Discuss.ApiView',
                        'Elixir.Discuss.AuthController',
                        'Elixir.Discuss.AuthView',
                        'Elixir.Discuss.ChangesetView',
                        'Elixir.Discuss.Channel','Elixir.Discuss.Comment',
                        'Elixir.Discuss.Endpoint',
                        'Elixir.Discuss.ErrorHelpers',
                        'Elixir.Discuss.ErrorView','Elixir.Discuss.Gettext',
                        'Elixir.Discuss.GuardianController',
                        'Elixir.Discuss.GuardianSerializer',
                        'Elixir.Discuss.LayoutView','Elixir.Discuss.Message',
                        'Elixir.Discuss.MySecretKey',
                        'Elixir.Discuss.PageController',
                        'Elixir.Discuss.PageView',
                        'Elixir.Discuss.Plugs.RequireAuth',
                        'Elixir.Discuss.Plugs.SetUser','Elixir.Discuss.Repo',
                        'Elixir.Discuss.Router',
                        'Elixir.Discuss.Router.Helpers',
                        'Elixir.Discuss.SlackUser',
                        'Elixir.Discuss.SlackUserController',
                        'Elixir.Discuss.SlackUserView','Elixir.Discuss.Topic',
                        'Elixir.Discuss.TopicController',
                        'Elixir.Discuss.TopicView','Elixir.Discuss.User',
                        'Elixir.Discuss.UserSocket','Elixir.Discuss.Web',
                        'Elixir.Poison.Encoder.Discuss.Channel',
                        'Elixir.Poison.Encoder.Discuss.Comment',
                        'Elixir.Poison.Encoder.Discuss.Message',
                        'Elixir.Poison.Encoder.Discuss.SlackUser',
                        'Elixir.Poison.Encoder.Discuss.Topic',
                        'Elixir.Poison.Encoder.Discuss.User',
                        'Elixir.Poison.Encoder.Ecto.Association.NotLoaded']},
              {registered,[]},
              {vsn,"0.0.1"},
              {mod,{'Elixir.Discuss',[]}},
              {applications,[kernel,stdlib,elixir,phoenix,phoenix_pubsub,
                             phoenix_html,cowboy,logger,gettext,phoenix_ecto,
                             postgrex,ueberauth,ueberauth_github,jose,
                             poison]}]}.
